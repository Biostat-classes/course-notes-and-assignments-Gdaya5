...
The : means that it makes a list in a sequence from the bottom number to
the top.
...
length returns the number of items or elements in a vector.

```{r}
max(x)
```

The max returns the highest value.
...
Square brackets allows you to return ( or work with) only portions of a
vector

#### 3

Is `-1:2` the same as `(-1):2` or `-(1:2)`?
...
The first creates a sequence from -1 to 2 by ones, the second one does
the same And the third creates a sequence from 1 to 2 then applies a
negative to it.
...
dataframe! So sleep has rdim(sleep)\[1\] rows.
...
We have a few measurements where BrainWt\>1000, so let's exclude those
for the model.
...
We see a significant (p \< .05) relationship between BrainWt and
TotalSleep, and it appears that TotalSleep decreases as BrainWt
